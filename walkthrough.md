...
## 12. 로컬 AI 설정 적용 오류 해결 및 테넌트 복합 키 쿼리 고도화 (완료)
* **원인 규명**: 
  - 웹 설정 화면에서 로컬 AI(`local_llm`) 공급자와 관련 정보들을 설정하고 저장하면, 다중 테넌트 격리를 지원하는 복합 키(`tenantId:key`) 형식(예: `tenant-guest-id-2222:ai_provider`)으로 `system_settings` 테이블에 영구 저장됩니다.
  - 하지만 전사 AI 라우팅의 관문인 `src/lib/ai-router.ts` 내 `callAI` 함수에서는 테넌트 컨텍스트 없이 단순 키(`ai_provider`, `local_llm_url` 등)를 그대로 조회함으로써 DB에서 설정을 찾지 못해(0건), 항상 기본값인 `gemini` (`gemini-3.5-flash`) 모델로 자동 풀백되어 실행되는 오류가 존재했습니다.
* **해결 및 구현 사항**:
  - `src/lib/ai-router.ts` 파일 내에 `getTenantId` 모듈을 연동하여 현재 사용자 로그인 세션의 `tenantId`를 동적으로 가져올 수 있도록 개선했습니다. (쿠키 접근이 불가능한 예외적 non-HTTP 환경에 대비하여 `try-catch` 가드를 완비했습니다.)
  - 조회 성능 최적화 및 안정적인 하위 호환을 위해 **3단계 다단계 폴백(Fallback) 조회 구조**를 설계 및 도입했습니다:
    1. **1단계**: 현재 사용자 테넌트 격리 복합 키 조회 (`tenantId:key`)
