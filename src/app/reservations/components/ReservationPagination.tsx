    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden mt-4 p-4 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50/50">
      <div className="flex items-center gap-2">
        ...
        <select 
          ...
          className="border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs outline-none bg-white font-bold cursor-pointer text-slate-700 focus:border-indigo-500 transition-colors"
        >
      ...
      <div className="flex items-center gap-1">
        ...
        {totalPages <= 1 ? (
          <button 
            type="button"
            disabled 
            className="px-3 py-1.5 rounded-lg text-xs font-bold bg-indigo-500 text-white shadow-sm disabled:opacity-50 cursor-not-allowed border-0"
          >
            1
          </button>
        ) : (
          Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
            <button 
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                currentPage === page 
                  ? "bg-indigo-500 text-white shadow-sm border-indigo-500 border-0" 
