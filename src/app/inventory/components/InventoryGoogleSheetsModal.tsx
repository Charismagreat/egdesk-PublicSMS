...
import { 
  Globe, Package, AlertCircle, CheckCircle2, X, Loader2, RefreshCw, Check, Layers, ShieldCheck, AlertTriangle, Bookmark, List
} from "lucide-react";
...
export default function InventoryGoogleSheetsModal({
  ...
  const fetchPresetsList = async () => {
    try {
      const res = await apiFetch("/api/shared/google-sheets/presets?domain=inventory");
