...
import { 
  Globe, AlertCircle, CheckCircle2, X, RefreshCw, Sparkles, Download, ExternalLink, Info, Bookmark, List
} from "lucide-react";
...
interface ProductGoogleSheetsUploadModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function ProductGoogleSheetsUploadModal({
  isOpen,
  onClose,
  onImport
}: ProductGoogleSheetsUploadModalProps) {
  ...
  const fetchPresetsList = async () => {
    try {
      const res = await apiFetch("/api/shared/google-sheets/presets?domain=product");
  ...
  useEffect(() => {
    if (isOpen) {
      setSheetUrl(getSavedGoogleSheetUrl('product_inbound_sheet_url') || getSavedGoogleSheetUrl());
      setParsedProducts([]);
      setValidationErrors([]);
      setStatusMsg(null);
  ...
  const handleFetchSheet = async (overrideSheetName?: string) => {
    ...
    try {
      setSavedGoogleSheetUrl('product_inbound_sheet_url', sheetUrl.trim());
      ...
      rawRows.forEach((row: any, idx: number) => {
        ...
        if (!mappedItem.name) {
          errorList.push(`[${rowNum}행] 상품명이 누락되었습니다.`);
          return;
        }
      ...
      if (validList.length > 0) {
        setStatusMsg({ 
          type: 'success', 
          text: `총 ${validList.length}건의 상품 데이터를 성공적으로 분석했습니다.` 
        });
      } else {
        setStatusMsg({ 
          type: 'error', 
          text: '유효한 상품 데이터가 없습니다. 필수 컬럼(상품명)을 확인하세요.' 
        });
      }
    } catch (e: any) {
  ...
  const handleExecuteImport = async () => {
    ...
    try {
      const res = await onImport(parsedProducts);
      if (res.success) {
        alert(`🎉 총 ${res.count || parsedProducts.length}건의 상품이 성공적으로 일괄 등록되었습니다!`);
        handleClose();
      } else {
    ...
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl max-w-2xl w-full p-6 shadow-2xl border border-slate-100 flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 font-semibold mt-0.5">공유된 구글 스프레드시트 링크를 통해 상품 데이터를 1초 만에 불러옵니다.</p>
            </div>
          </div>
          ...
          {parsedProducts.length > 0 && (
            <div className="space-y-1.5 animate-scale-up">
              ...
              <div className="space-y-1.5 max-h-48 overflow-y-auto">
                {parsedProducts.slice(0, 5).map((prod, idx) => (
                  <div key={idx} className="bg-white p-3 rounded-xl border border-slate-200/80 flex items-center justify-between text-xs font-semibold">
                    ...
                    <div className="text-right shrink-0">
                      <span className="text-blue-600 font-mono font-black">{prod.price.toLocaleString()}원</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="mt-4 border-t border-slate-100 pt-4 flex gap-3 shrink-0">
          ...
          >
            {parsedProducts.length > 0 ? `🚀 총 ${parsedProducts.length.toLocaleString()}개 상품 일괄 등록 완료` : "시트 데이터 로드 대기 중"}
          </button>
        </div>
      </div>
