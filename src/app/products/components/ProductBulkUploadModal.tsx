...
import { X, UploadCloud, CheckCircle2, AlertCircle, FileSpreadsheet, Info, Download, Sparkles } from "lucide-react";
import * as XLSX from "xlsx";

interface ProductBulkUploadModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImport: (products: any[]) => Promise<{ success: boolean; count?: number; error?: string }>;
}
...
const HEADER_MAPPING: Record<string, string> = {
  ...
  "상세설명": "description",
  "설명": "description",
  "메인이미지URL": "main_image_url",
  "이미지URL": "main_image_url",
  "대표이미지": "main_image_url",
  "상세페이지URL": "url",
  "상품링크": "url",
  "URL": "url"
};

export function ProductBulkUploadModal({ isOpen, onClose, onImport }: ProductBulkUploadModalProps) {
  ...
  const processFile = (file: File) => {
    ...
    setParsedData([]);
    setErrors([]);
    setImportStatus("파일을 분석하고 상품 정보를 추출하는 중입니다...");
    ...
    reader.onload = (e) => {
      try {
        ...
        rawRows.forEach((row: any, idx: number) => {
          ...
          if (!mappedItem.name) {
            localErrors.push(`[${rowNum}행] 상품명이 누락되었습니다.`);
            return;
          }

          const priceNum = Number(String(mappedItem.price || "0").replace(/[^0-9.-]+/g, "")) || 0;

          validProducts.push({
            name: mappedItem.name,
            category: mappedItem.category || "일반상품",
            price: priceNum,
            brand: mappedItem.brand || "",
            spec: mappedItem.spec || "",
            unit: mappedItem.unit || "개",
            description: mappedItem.description || "",
            main_image_url: mappedItem.main_image_url || "",
            url: mappedItem.url || ""
          });
        });
  ...
  const handleExecuteImport = async () => {
    ...
    setLoading(true);
    setImportStatus(`총 ${parsedData.length}건의 상품 정보를 데이터베이스에 일괄 적재 중입니다...`);

    try {
      const res = await onImport(parsedData);
      if (res.success) {
        alert(`🎉 총 ${res.count || parsedData.length}건의 상품이 성공적으로 일괄 등록되었습니다!`);
        handleClose();
      } else {
  ...
  const handleDownloadTemplate = () => {
    ...
    link.href = "/이지데스크-표준- 상품 등록 양식.xlsx";
    link.download = "이지데스크-표준- 상품 등록 양식.xlsx";
    link.click();
  };
      ...
      <div className="bg-white rounded-3xl max-w-2xl w-full p-6 shadow-2xl border border-slate-100 flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-lg font-black text-slate-800 tracking-tight">상품 엑셀 업로드</h3>
              <p className="text-xs text-slate-400 font-semibold mt-0.5">엑셀 파일(.xlsx, .csv)을 업로드하여 다수의 상품을 한 번에 일괄 등록합니다.</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-4 pr-1">
          ...
          <div className="flex items-center justify-between bg-emerald-50/70 border border-emerald-200/80 p-3.5 rounded-2xl">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-emerald-950">시스템 표준 서식을 다운로드하여 상품 명세를 쉽게 정리하세요.</span>
            </div>
            <button
          ...
          {!fileName && (
            ...
            >
              ...
              <p className="text-xs font-bold text-slate-700">상품 엑셀 또는 CSV 파일을 여기로 드래그하거나 클릭하여 선택하세요</p>
            ...
            <div className="space-y-4 animate-scale-up">
                ...
                <div className="p-3.5 bg-emerald-50 border border-emerald-200 rounded-2xl flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-black text-emerald-900">
                      총 {parsedData.length.toLocaleString()}건의 유효한 상품 정보가 확인되었습니다.
                    </span>
                  </div>
                </div>
              )}
              ...
              {parsedData.length > 0 && (
                <div className="space-y-1.5">
                  <span className="text-[11px] font-bold text-slate-500 block">상품 미리보기 (상위 5건)</span>
                  <div className="space-y-1.5 max-h-48 overflow-y-auto">
                    {parsedData.slice(0, 5).map((prod, idx) => (
                      <div key={idx} className="bg-white p-3 rounded-xl border border-slate-200/80 flex items-center justify-between text-xs font-semibold">
                        <div className="flex items-center gap-2 truncate">
                          <span className="px-1.5 py-0.5 bg-slate-100 text-slate-700 text-[10px] rounded font-bold shrink-0">
                            {prod.category}
                          </span>
                          <span className="font-bold text-slate-800 truncate">{prod.name}</span>
                          {prod.spec && (
                            <span className="text-slate-400 font-normal text-[11px] truncate">({prod.spec})</span>
                          )}
                        </div>
                        <div className="text-right shrink-0">
                          <span className="text-emerald-700 font-mono font-black">{prod.price.toLocaleString()}원</span>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <div className="mt-4 border-t border-slate-100 pt-4 flex gap-3 shrink-0">
          ...
          >
            {parsedData.length > 0 ? `🚀 총 ${parsedData.length.toLocaleString()}개 상품 일괄 등록 완료` : "엑셀 파일 업로드 대기 중"}
          </button>
        </div>
      </div>
    </div>,
