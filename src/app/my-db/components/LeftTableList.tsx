...
import { Table as TableIcon, Search, X, Database, Loader2 } from "lucide-react";
  ...
  return (
    <div className="xl:col-span-1 bg-white rounded-2xl shadow-sm border border-slate-100 p-5 overflow-hidden flex flex-col">
      <h2 className="font-extrabold text-slate-800 text-base pb-3.5 border-b border-slate-100 flex items-center gap-2 mb-3 shrink-0">
        ...
        물리 테이블 {isTablesLoading ? "(로딩 중...)" : `(${filteredTables.length}/${tables.length})`}
      </h2>
      ...
      <div className="relative mb-3.5 shrink-0">
        <input
          ...
          className="w-full pl-8 pr-7 py-2 text-xs bg-slate-50 hover:bg-slate-100/50 focus:bg-white text-slate-705 placeholder-slate-400 border border-slate-250 focus:border-blue-400 focus:ring-1 focus:ring-blue-100 rounded-xl outline-none transition-all font-semibold disabled:opacity-60 disabled:cursor-not-allowed"
        />
