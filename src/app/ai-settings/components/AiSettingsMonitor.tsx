...
interface AiSettingsMonitorProps {
  range: "today" | "week" | "month" | "all";
  setRange: (r: "today" | "week" | "month" | "all") => void;
  ...
  handleToggleTableCollapse: () => void;
}

export function AiSettingsMonitor({
  range,
  setRange,
  summary,
  purposes,
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-5 gap-4">
          <div>
            <h2 className="text-lg font-extrabold text-slate-800 flex items-center gap-2">
              <Cpu className="w-5.5 h-5.5 text-indigo-650" />
              AI API 토큰 실시간 모니터링 대시보드
            </h2>
            <p className="text-xs text-slate-400 mt-1 leading-relaxed">
              구글 Gemini API 및 로컬 LLM을 통해 시스템에서 소모되는 인공지능 토큰 소모량을 실시간 정밀 감사 분석합니다.
            </p>
          </div>
            ...
            {[
              ...
              <button
                ...
                className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all cursor-pointer ${
                  range === r.id ? "bg-white text-indigo-650 shadow-xs" : "text-slate-500 hover:text-slate-850"
                }`}
              >
                {r.label}
              </button>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
              ...
              {/* 총 토큰 소모량 */}
              <div className="bg-slate-50/50 border border-slate-150 rounded-2xl p-4 flex items-center justify-between shadow-2xs">
                <div>
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">총 소모 토큰량</p>
                  <h4 className="text-xl font-black text-slate-800 mt-1">
                    {summary.total_tokens.toLocaleString()} <span className="text-xs font-medium text-slate-400">t</span>
                  </h4>
                </div>
              ...
              {/* 예상 비용 */}
              <div className="bg-slate-50/50 border border-slate-150 rounded-2xl p-4 flex items-center justify-between shadow-2xs col-span-2 sm:col-span-1">
                <div>
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">예상 API 사용 비용</p>
                  <h4 className="text-xl font-black text-slate-800 mt-1">
              ...
              <div className="bg-slate-50/50 border border-slate-150 rounded-2xl p-4 flex items-center justify-between shadow-2xs">
                <div>
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">AI API 호출 횟수</p>
                  <h4 className="text-xl font-black text-slate-800 mt-1">
                    {summary.api_calls.toLocaleString()} <span className="text-xs font-medium text-slate-400">회</span>
                  </h4>
                </div>
              ...
              <div className="bg-slate-50/50 border border-slate-150 rounded-2xl p-4 flex items-center justify-between shadow-2xs">
                <div>
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">입력(질문) 토큰량</p>
                  <h4 className="text-xl font-black text-slate-800 mt-1">
                    {summary.total_prompt_tokens.toLocaleString()} <span className="text-xs font-medium text-slate-400">t</span>
                  </h4>
                </div>
              ...
              <div className="bg-slate-50/50 border border-slate-150 rounded-2xl p-4 flex items-center justify-between shadow-2xs">
                <div>
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">출력(답변) 토큰량</p>
                  <h4 className="text-xl font-black text-slate-800 mt-1">
                    {summary.total_completion_tokens.toLocaleString()} <span className="text-xs font-medium text-slate-400">t</span>
                  </h4>
                </div>
                      ...
                      return purposes.map(p => {
                        ...
                        return (
                          <div key={p.purpose} className="space-y-1">
                            ...
                            <div className="w-full bg-slate-200/60 h-3 rounded-full overflow-hidden p-0.5 border border-slate-200/50">
                              <div
                        ...
                        return (
                          <div key={m.model} className="space-y-1">
                            <div className="flex justify-between text-xs font-bold text-slate-750">
                              <span className="font-mono text-indigo-950 truncate max-w-[220px]">{m.model}</span>
                              <span className="shrink-0 flex items-center gap-1.5 font-mono">
            ...
            {/* 4. 실시간 AI 감사 내역 테이블 */}
            <div className="border border-slate-150 rounded-2xl overflow-hidden mt-2 transition-all duration-300">
              <div 
                onClick={handleToggleTableCollapse}
                className="p-4 bg-slate-50 border-b border-slate-150 flex items-center justify-between cursor-pointer hover:bg-slate-100/50 transition-colors select-none"
              ...
              {!isTableCollapsed && (
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs border-collapse whitespace-nowrap">
                    <thead>
                      <tr className="bg-slate-50/50 border-b border-slate-150 text-slate-500 font-bold">
                        ...
                        <th className="p-3 text-right bg-slate-550/10">총 소모량</th>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {recentLogs.length === 0 ? (
                        <tr>
                          <td colSpan={9} className="p-8 text-center text-slate-450 font-semibold bg-white">
                            기록된 실시간 AI 호출 이력이 없습니다. 이지봇 대화나 자동 마케팅을 가동해 보세요!
                          </td>
                        </tr>
                      ) : (
                        recentLogs.map(log => (
                          <tr key={log.id} className="hover:bg-slate-50/30 bg-white transition-colors">
                  ...
                  {/* 5. 페이지네이션 바 */}
                  {recentLogs.length > 0 && (
                    <div className="p-4 bg-slate-50/50 border-t border-slate-150 flex flex-col sm:flex-row items-center justify-between gap-4 select-none">
                      <div className="text-[11px] text-slate-500 font-semibold order-2 sm:order-1">
                        총 <span className="text-indigo-650 font-extrabold">{pagination.total.toLocaleString()}</span>건 중{" "}
                      ...
                      {pagination.totalPages > 1 && (
                        <div className="flex items-center gap-1 order-1 sm:order-2">
                          ...
                          <button
                            ...
                            className="px-2.5 py-1 text-slate-500 bg-white border border-slate-200 rounded-lg text-[11px] font-bold hover:bg-slate-55 transition-colors disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
                          >
                            다음
                          </button>
                          <button
                            ...
                            disabled={page === pagination.totalPages}
                            className="px-2 py-1 text-slate-500 bg-white border border-slate-200 rounded-lg text-[10px] font-bold hover:bg-slate-55 transition-colors disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
                            title="마지막 페이지"
                          >
                            &gt;&gt;
                          </button>
