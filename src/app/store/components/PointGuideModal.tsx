...
interface PointGuideModalProps {
  showPointGuide: boolean;
  setShowPointGuide: (val: boolean) => void;
  pointEarningRate: number;
}

export function PointGuideModal({
  showPointGuide,
  setShowPointGuide,
  pointEarningRate
}: PointGuideModalProps) {
  if (!showPointGuide) return null;
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-6 animate-fade-in">
      ...
      <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl relative z-10 overflow-hidden flex flex-col p-6 sm:p-8 animate-scale-up border border-slate-100/50">
        <button 
          ...
          className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 p-2 rounded-full hover:bg-slate-100 transition-colors border-none bg-transparent cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <button 
          onClick={() => setShowPointGuide(false)} 
          className="mt-6 w-full py-3.5 bg-slate-900 text-white font-bold rounded-xl text-xs hover:bg-slate-800 transition-all border-none shadow-md shadow-slate-900/10 cursor-pointer"
        >
          확인 및 닫기
        </button>
      </div>
