  ...
  const handleOcrFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        setOcrScanStep("3단계: 정방향 문서를 기반으로 발주서 상세 내역을 추출하는 중...");
        ...
        if (data.success) {
          ...
          const parsedItems = (data.items || []).map((it: any) => {
            ...
            // 품명 및 규격 가공 조립
            let nameWithSpec = it.product_name || "품목명 없음";
            if (it.spec) {
              nameWithSpec = `${nameWithSpec} (${it.spec})`;
            }

            return {
              item_code: it.item_code || it.itemCode || "",
              product_name: nameWithSpec,
              quantity: qty,
              unit_price: price,
              amount: amt,
          ...
          <div className="border-2 border-dashed border-slate-200 rounded-2xl p-6 flex flex-col items-center justify-center bg-slate-50 relative min-h-[140px] overflow-hidden">
            ...
            {ocrScanning ? (
              <div className="flex flex-col items-center space-y-2 text-center">
                ...
                <span className="text-xs text-indigo-600 font-extrabold animate-pulse">{ocrScanStep || "Gemini Vision AI가 공급처 원본을 자동 분류 및 고해상도 판독 중..."}</span>
              </div>
            ) : ocrSuccess ? (
