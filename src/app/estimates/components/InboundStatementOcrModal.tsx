...
export default function InboundStatementOcrModal({
  ...
  // 📝 단일 폼 폴백 상태
  const [ocrForm, setOcrForm] = useState<ParsedStatementGroup>({
    id: "single",
    partner_name: "",
    partner_phone: "",
    partner_manager: "",
  ...
  const parseTableDataToStatement = async (rawRows: any[][], sourceTitle: string) => {
    ...
    const groupsMap = new Map<string, ParsedStatementGroup>();
    let fallbackPartner = sourceTitle.replace(/\.[^/.]+$/, "").replace(/[-_]/g, " ").trim() || "공급사";
    ...
    if (groups.length === 0) {
      throw new Error("유효한 거래명세서 품목 데이터를 추출하지 못했습니다.");
    }
  ...
  const handleExcelFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await parseTableDataToStatement(combinedRows, file.name);
    } catch (err: any) {
      setOcrScanning(false);
    ...
    reader.onload = async () => {
      ...
      try {
        const res = await apiFetch("/api/estimates/ocr", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ imageBase64: base64Data, filename: file.name, document_type: "statement" })
        });
        const data = await res.json();
        if (data.success) {
      ...
      <div className="bg-white rounded-3xl max-w-3xl w-full p-6 shadow-2xl border border-slate-100 flex flex-col max-h-[92vh]">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-2.5">
            <span className="p-2.5 bg-amber-50 text-amber-600 rounded-2xl shadow-3xs">
          ...
          {ocrSuccess && (
            <div className="bg-slate-50 p-4.5 rounded-2xl border border-slate-100 space-y-4 animate-scale-up">
              ...
              {parsedGroups.length > 1 && (
                <div className="p-3.5 bg-amber-500/10 border border-amber-200 rounded-2xl flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="p-2 bg-amber-600 text-white rounded-xl shadow-2xs">
        ...
        <div className="mt-6 border-t border-slate-100 pt-4 flex gap-3 shrink-0">
          ...
          <button 
            onClick={handleSaveAllStatements}
            disabled={!ocrSuccess}
            className="flex-1 py-3 bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs rounded-xl disabled:opacity-50 transition-colors cursor-pointer shadow-md active:scale-95"
          >
            {parsedGroups.length > 1 
              ? `🚀 총 ${parsedGroups.length}건의 거래명세서 일괄 등록 승인` 
              : "받은 거래명세서 등록 승인"}
          </button>
        </div>
      </div>
