...
export const getCategoryBadgeStyle = (mainCat: string): string => {
  switch (mainCat) {
    ...
    case "영업외비용":
      return "bg-amber-50 text-amber-700 border border-indigo-100/60";
    default:
...
export const downloadAccountsExcel = async (transactionList: Transaction[]) => {
  ...
  try {
    ...
    XLSX.writeFile(workbook, `금융관리_은행거래내역_${new Date().toISOString().slice(0, 10)}.xlsx`);
  } catch (e: any) {
    alert("엑셀 파일 생성 중 오류가 발생했습니다: " + e.message);
  }
};
...
export const downloadCardsExcel = async (cardTxList: CardTransaction[]) => {
  ...
  try {
    ...
    XLSX.writeFile(workbook, `금융관리_카드승인내역_${new Date().toISOString().slice(0, 10)}.xlsx`);
  } catch (e: any) {
    alert("엑셀 파일 생성 중 오류가 발생했습니다: " + e.message);
  }
};
...
export const downloadHometaxInvoiceExcel = async (list: HometaxInvoice[], isExempt: boolean) => {
  ...
  try {
    ...
    XLSX.writeFile(workbook, `금융관리_국세청_${title}_${new Date().toISOString().slice(0, 10)}.xlsx`);
  } catch (e: any) {
    alert("엑셀 파일 생성 중 오류가 발생했습니다: " + e.message);
  }
};
...
export const downloadHometaxCashExcel = async (cashReceiptList: HometaxCash[]) => {
  ...
  try {
    ...
    XLSX.writeFile(workbook, `금융관리_현금영수증_${new Date().toISOString().slice(0, 10)}.xlsx`);
  } catch (e: any) {
    alert("엑셀 파일 생성 중 오류가 발생했습니다: " + e.message);
  }
};
