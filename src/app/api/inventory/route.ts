    ...
    try {
      if (tenantId !== 'tenant-guest-id-2222') {
        while (true) {
          ...
          const ids = wrongInvRows.map((r: any) => Number(r.id));
          await updateRows('inventory_items', { tenant_id: tenantId }, { ids });
          console.log(`[Self-Healing] Migrated ${ids.length} inventory items to current tenant: ${tenantId}`);
          
          if (wrongInvRows.length < 1000) break;
        }
      } else {
        while (true) {
