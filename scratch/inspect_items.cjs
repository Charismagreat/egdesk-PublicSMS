...
async function inspectItems() {
  const itemsRes = await queryTable('crm_snaptask_items', { limit: 200 });
  const rows = itemsRes.rows || [];
  console.log("=== crm_snaptask_items 전체 레코드 수:", rows.length);
  const targetRows = rows.filter((r) => String(r.task_id).includes("1785296513094") || String(r.id).includes("1785296513094"));
