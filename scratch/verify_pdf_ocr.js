...
async function verify() {
  ...
  try {
    ...
    selectedModel = modelRow.rows && modelRow.rows.length > 0 ? modelRow.rows[0].value : 'gemini-2.5-flash';
  } catch (err) {
  ...
  // uploads/financials 폴더 내 가장 최근 생성된 pdf 읽기
  const uploadDir = path.join(__dirname, '..', 'public', 'uploads', 'financials');
  const files = fs.readdirSync(uploadDir).filter(f => f.endsWith('.pdf'));
  if (files.length === 0) {
    console.error('PDF 파일이 존재하지 않습니다.');
    return;
  }

  files.sort((a, b) => {
    return fs.statSync(path.join(uploadDir, b)).mtime.getTime() - fs.statSync(path.join(uploadDir, a)).mtime.getTime();
  });

  const latestPdf = files[0];
  const pdfPath = path.join(uploadDir, latestPdf);
  console.log(`최근 PDF 파일 로드: ${latestPdf} (${fs.statSync(pdfPath).size} bytes)`);
