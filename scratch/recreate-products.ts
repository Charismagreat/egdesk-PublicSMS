...
async function recreate() {
  ...
  try {
    await executeSQL('ALTER TABLE products ADD COLUMN category TEXT');
  ...
  try {
    await executeSQL('ALTER TABLE products ADD COLUMN available_methods TEXT');
    console.log('Added available_methods column');
  } catch(e) {
    console.log('available_methods exists');
  }
  
  // Now clear the table so we can re-seed
  try {
